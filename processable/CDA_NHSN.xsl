...
<!--
  ...
  Revision History: 06/13/14 Sarah Gaunt - added NHSN logo
  Revision History: 02/15/18 Rick Geimer. Minor formatting fix to authenticator.
  Revision History: 02/15/18 Rick Geimer. Fixed some iframe issues (attribute typos). Added datauri support for nonXMLBody docs with B64 PDFs (does not work in IE or Edge as of this time, but does work in Chrome and FireFox).
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3"
    ...
    <xsl:variable name="javascript-injection-warning">WARNING: Javascript injection attempt detected in source CDA document.
            ...
            <xsl:for-each select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole">
                <xsl:if test="*">
                    <tr>
                        ...
                        <td class="td_header_role_value">
                            <xsl:choose>
                                <xsl:when test="n1:patient/n1:name">
                                    <xsl:call-template name="show-name">
                                        <xsl:with-param name="name" select="n1:patient/n1:name"/>
                                    </xsl:call-template>
                                </xsl:when>
                            <xsl:otherwise>
                                    <xsl:text>Name not available</xsl:text>
                                </xsl:otherwise>
                            </xsl:choose>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="td_header_role_value">
                            <xsl:choose>
                                <xsl:when test="n1:patient/n1:birthTime">
                                    <xsl:call-template name="show-time">
                    ...
                    <tr>
                        ...
                        <td class="td_header_role_value">
                            <xsl:choose>
                                <xsl:when test="n1:patient/n1:administrativeGenderCode">
...
-->
    ...
    <xsl:template name="check-external-image-whitelist">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <p>WARNING: non-local image found <xsl:value-of select="$image-uri"/>. Removing. If you wish non-local images preserved please set the
                    limit-external-images param to 'no'.</p>
                <xsl:message>WARNING: non-local image found <xsl:value-of select="$image-uri"/>. Removing. If you wish non-local images preserved
                    please set the limit-external-images param to 'no'.</xsl:message>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
