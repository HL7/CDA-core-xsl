...
- Although patient demographic info (such as name and address) is part of USCDI, the stylesheet does not add provenance tooltips for this info because CDA doesn't provide a means to associate an author with document header information at the data element level. The document-level author is rendered alongside the demographic information in the header and may or may not be considered a source of provenance for these elements.
- Due to limitations in Internet Explorer, elements with associated provenance have solid (not dotted) underlines in it, which may create ambiguity with the Underline `styleCode`. Users are encouraged to upgrade to a modern browser.
## Manual
